...
test("database routes fail closed before configuration", async () => {
  ...
  const clientReferenceDataResponse = await server.inject({
    method: "GET",
    url: "/v1/admin/client-reference-data"
  });
  const appointmentsResponse = await server.inject({
    method: "GET",
    url: "/v1/admin/appointments"
  });
  const rescheduleAvailabilityResponse = await server.inject({
    method: "GET",
    url: "/v1/admin/appointments/cmh0000000000000000000000/availability?date=2026-09-24"
  });
  ...
  const availabilityResponse = await server.inject({
    method: "GET",
    url: "/v1/availability?staffId=cmh0000000000000000000000&serviceId=cmh0000000000000000000000&date=2026-09-24"
  });
  ...
  assert.equal(clientReferenceDataResponse.statusCode, 503);
  assert.equal(appointmentsResponse.statusCode, 503);
  assert.equal(rescheduleAvailabilityResponse.statusCode, 503);
