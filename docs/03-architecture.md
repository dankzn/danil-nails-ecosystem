...
```text
apps/
  crm/              # закрытая CRM для внутренних ролей
...
- `owner_founder` и `owner_cofounder` видят все данные и функции CRM; роли разделены для идентификации и аудита.
- `smm_manager` видит только главную страницу и отдельный ограниченный набор агрегированных виджетов без доступа к исходным сущностям.
- `salon_admin` управляет клиентами и операционным циклом записей, просматривает услуги и работает со своей кассовой сменой; кадровые, зарплатные и системные разделы запрещены.
- Client является внешним пользователем и видит только свои записи, публичный статус лояльности и разрешённые данные профиля.
- Отдельная роль master проектируется для будущего найма и будет ограничена собственными записями и необходимой частью клиентской карточки; на текущем этапе она не активна.
- Авторизация применяется на уровне маршрута, операции и полей ответа. По умолчанию доступ запрещён, а неописанные разрешения не наследуются от видимости интерфейса.
